...
namespace SistemaWebPedidos.Application.Services
{
    public class SobreService : ISobreService
    {
        ...
        {
            return _mapper.Map<SobreViewModel>( await _sobreRepository.ObterTodos());
        }

        public async Task<SobreViewModel> Salvar(SobreViewModel sobreViewModel)
        {
            var sobre = _mapper.Map<Sobre>(sobreViewModel);
            return _mapper.Map<SobreViewModel>( await _sobreRepository.Adcionar(sobre));
